...
- Type and lifecycle safety that is ensured at compile time also makes dynamic dependency injection an impossible and futile attempt (aside from macro rules which essentially auto-generate codes with constructors). With Golang dynamic DI is still somewhat viable (see runway / platform repo, inverse module)
